  ...
  return (
    <div className="flex min-h-screen bg-[#f0f5ff] font-sans text-slate-900">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 flex h-16 border-t border-slate-200 bg-white/95 backdrop-blur-md px-2 shadow-[0_-4px_16px_rgba(0,0,0,0.06)] lg:hidden pb-safe">
        <BottomNavItem href={`/dashboard/student/${studentId}`} icon={<Home size={18} />} label="Beranda" active={pathname === `/dashboard/student/${studentId}`} />
        <BottomNavItem href={`/dashboard/student/${studentId}/journals`} icon={<BookOpen size={18} />} label="Modul" active={pathname === `/dashboard/student/${studentId}/journals`} />
        <BottomNavItem href={`/dashboard/student/${studentId}/riasec`} icon={<Sparkles size={18} />} label="RIASEC" active={pathname === `/dashboard/student/${studentId}/riasec`} />
        <BottomNavItem href={`/dashboard/student/${studentId}/schedule`} icon={<Calendar size={18} />} label="Konseling" active={pathname === `/dashboard/student/${studentId}/schedule`} />
