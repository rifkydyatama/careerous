...
export default function AdminDashboard() {
  ...
  const planData = useMemo(() => {
    ...
    return [
      { name: "Premium", value: stats.premiumUsers, color: "#C9920A" },
...
function QuickLink({ href, icon, title, desc }: { href: string; icon: React.ReactNode; title: string; desc: string }) {
  return (
    <Link
      href={href}
      className="group flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-[#0B1D3A]/30 hover:shadow-md"
    >
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#0B1D3A]/5 text-[#0B1D3A]">{icon}</div>
      <div className="flex-1">
        ...
        <span className="mt-2 inline-flex items-center gap-1 text-[11.5px] font-bold text-[#0B1D3A]">
